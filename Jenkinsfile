pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/Sudojamal/JenkinsGit.git']])
            }
        }
         stage('Test') {
            steps {
                git branch: 'main', url: 'https://github.com/Sudojamal/JenkinsGit.git'
